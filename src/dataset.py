...
class RulesDataset(Dataset):
    def __init__(self, base_path, keypoint_num=26, std_h=1, std_w=1):
        ...
        self.base_path = base_path
        self.ip = ImageProcess(base_path)
        self.img_data = list(self.ip.get_rule_data())
        self.data_len = len(self.img_data)

    def __getitem__(self, index):
        img_name, keypoints, distance, angle = self.img_data[index]
        keypoints = torch.Tensor(keypoints)
        distance = torch.Tensor(distance)
        angle = torch.Tensor(angle)
        return NAME_MAP[img_name], keypoints[:, :2].flatten(), distance, angle
